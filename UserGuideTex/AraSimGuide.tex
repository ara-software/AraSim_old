...
Adding lines like this: `\p{PARAMETER}= N' will set the appropriate parameter to N. The order of input lines is irrelevant. 
...
For $1<N<4$ the distribution is just $E^{-N}$. Choosing $N\geq20$ gives energies based on different models (see list\footnote{Work in progress. Use `README\textunderscore~EXPONENT.pdf' for more details}).
...
\pr{NOISE}=0 (default) generates waveforms from a flat thermal distribution. mode 1 (TB only) uses calibrated Rayleigh distribution noise from testbed data. 

\pr{NOISE TEMP MODE}=0 (default) generate the same noise waveforms for all channels. When using \p{NOISE}= 1 (Rayleigh distribution), we can choose several modes for the \tb{testbed only}. mode 1 generate different noise temperatures for each channel. mode 2 generates different noise for the first 8 channels (borehole antennas) and the rest are all the same.  
...
\pr{TRIG THRES MODE}=0 (default) use the same trigger threshold for all channels. mode 1 gives each channel its own threshold offset from file `data/thresholdoffset.csv', currently \tb{testbed only}! 
...
\r{NOISE}{0: flat thermal distribution. 1: calibrated Rayleigh noise (\tb{testbed only!)}.}
...
\r[1]{INTERACTION MODE}{0: make nu's all over antarctica. 1: pick near (a cylinder around detector). 2: exact location at (353.55, 612.37, 707.1). }
...
\r[5000]{RAYSOL RANGE}{distance from detector above which we do not attemp a ray solution (there is no trigger check beyond this).}
