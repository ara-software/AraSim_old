...
void RaySolver::Solve_Ray (Interaction *Evt, Position &target, IceModel *antarctica, std::vector < std::vector <double> > &outputs) {
    int argc = 7;
    int arg_length = 40;
    char argv_tmp[argc][arg_length];
    char *argv[argc];
...
//-------------------------------------------------- 
	...
	if(showLabels){
		if(!paths.empty()){
			...
			<< std::endl;
                        solution_toggle = 1;
                        Evt->ray_solver_toggle = 1;
		}
		else {
			std::cout << "No solutions" << std::endl;
                        solution_toggle = 0;
                        Evt->ray_solver_toggle = 0;
                }
	}
	if(!dumpPaths){
		for(std::vector<RayTrace::TraceRecord>::const_iterator it=paths.begin(); it!=paths.end(); ++it){
			...
			<< std::endl;
                        ...
                        outputs.resize(3);
                        Evt->ray_outputs.resize(3);

                        outputs[0].push_back(it->pathLen);
                        outputs[1].push_back(it->launchAngle);
                        outputs[2].push_back(it->receiptAngle);
                        Evt->ray_outputs[0].push_back(it->pathLen);
                        Evt->ray_outputs[1].push_back(it->launchAngle);
                        Evt->ray_outputs[2].push_back(it->receiptAngle);
                        std::cout<<"outputs[0]["<<sol_no<<"] : "<<outputs[0][sol_no]<<"\n";
                        sol_no++;
